...
class Builder
{
    ...
    {
        $file = file(storage_path('app/parlamentares/testemunhos-parlamentares-juvenis.txt'));
    ...
    {
        $flags = $this->dataRepository->flagContest->allFlagsWithVotes()->reject(function ($flag) {
    ...
    {
        $articles = Article::orderBy('published_at', 'desc')->where('type', $type);
        ...
        foreach ($articles = $articles->get() as $article) {
            if ($article->image) {
                $article->image =
                    env('ARTICLE_IMAGE_URL_BASE') . DIRECTORY_SEPARATOR . $article->image;
